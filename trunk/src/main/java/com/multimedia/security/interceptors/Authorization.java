package com.multimedia.security.interceptors;
...
import com.multimedia.security.Config;
...
public class Authorization implements HandlerInterceptor{
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if (response.isCommitted()){
			logger.error("response has been allready commited; url="+request.getRequestURL());
			return true;
		}
        ...
        if (!TRUE.equals(request.getAttribute(ALREADY_PROCESSED))) {
			HttpSession session=request.getSession();//TODO: here sometimes is error hz why
            ...
            if (userSession==null){
                ...
                if (request.getParameter(logout_param)!=null){
                    ...
                    User userDB=securityService.getUser(userSession.getId());
					if (userDB==null){
	...
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex)
		throws Exception
	{
		this.securityService.userEntered(Utils.getCurrentUser(request));
	}
	...
	protected void userChanged(HttpServletRequest req,String msg){
		...
		req.getSession().removeAttribute(Config.USER_ATTRIBUTE);
	}
